...
func TestPgSQLStorageInsertRoom(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectExec("INSERT INTO rooms (.+) ON CONFLICT (.+) DO UPDATE SET (.+)").
	...
	mock.ExpectExec("INSERT INTO rooms_invites (.+) ON CONFLICT (.+) DO UPDATE SET (.+)").
		...
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec("INSERT INTO rooms_users (.+) ON CONFLICT (.+) DO UPDATE SET (.+)").
		WithArgs(room.RoomJID.String(), userJID.String(), occJID.String()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()
...
func TestPgSQLStorageFetchRoom(t *testing.T) {
	room := getTestRoom()
	rc := room.Config
	s, mock := newRoomMock()
...
func TestPgSQLStorageDeleteRoom(t *testing.T) {
	room := getTestRoom()
	s, mock := newRoomMock()
	mock.ExpectBegin()
...
func TestPgSQLStorageRoomExists(t *testing.T) {
	room := getTestRoom()
...
func newRoomMock() (*pgSQLRoom, sqlmock.Sqlmock) {
